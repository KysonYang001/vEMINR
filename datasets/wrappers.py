...
class ScaleDownsampled_degrade(Dataset):
    ...
    def __init__(self, dataset, inp_size=48, augment=False, scale=1):
        self.dataset = dataset
        self.inp_size = inp_size
        self.augment = augment
        self.scale = scale
    ...
    def __getitem__(self, idx):
        ...
        w_lr = round(h_lr * s)
        try:
            x0 = random.randint(0, img.shape[-2] - h_lr)
            y0 = random.randint(0, img.shape[-1] - w_lr)
            x1 = random.randint(0, img.shape[-2] - h_lr)
            y1 = random.randint(0, img.shape[-1] - w_lr)

            # clean
            crop_q = img[:, x0: x0 + h_lr, y0: y0 + w_lr]
            crop_k = img[:, x1: x1 + h_lr, y1: y1 + w_lr]
        except:
            pdb.set_trace()
...
class ScaleDownsampled_gaussian(Dataset):
    ...
    def __init__(self, dataset, inp_size=48, augment=False, sample_q=None, scale=1):
        self.dataset = dataset
        self.inp_size = inp_size
        self.augment = augment
        self.sample_q = sample_q
