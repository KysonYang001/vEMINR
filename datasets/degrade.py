...
def random_anisotropic_gaussian_kernel(device, batch=1, kernel_size=21, lambda_min=0.2, lambda_max=4.0):
    theta = torch.rand(batch).to(device) * math.pi * 0 + 90
    lambda_1 = torch.rand(batch).to(device) * (lambda_max - lambda_min) + lambda_min
    lambda_2 = torch.rand(batch).to(device) * (lambda_max / 8 - lambda_min / 8) + lambda_min / 8
...
def random_gaussian_kernel(batch, device, kernel_size=21, blur_type='iso_gaussian', sig_min=0.2, sig_max=4.0,
                           lambda_min=0.2, lambda_max=4.0):
    if blur_type == 'iso_gaussian':
        return random_isotropic_gaussian_kernel(device=device, batch=batch, kernel_size=kernel_size, sig_min=sig_min,
                                                sig_max=sig_max)
    elif blur_type == 'aniso_gaussian':
        return random_anisotropic_gaussian_kernel(device=device, batch=batch, kernel_size=kernel_size,
                                                  lambda_min=lambda_min, lambda_max=lambda_max)


def stable_gaussian_kernel(device, kernel_size=21, blur_type='iso_gaussian', sig=2.6, lambda_1=0.2, lambda_2=4.0,
                           theta=0):
    if blur_type == 'iso_gaussian':
        return stable_isotropic_gaussian_kernel(device=device, kernel_size=kernel_size, sig=sig)
    elif blur_type == 'aniso_gaussian':
        return stable_anisotropic_gaussian_kernel(device=device, kernel_size=kernel_size, lambda_1=lambda_1,
                                                  lambda_2=lambda_2, theta=theta)
...
class bicubic(nn.Module):
    ...
    def contribute(self, in_size, out_size, scale):
        ...
        indice0 = left0.unsqueeze(1) + torch.arange(start=0, end=P).to(torch.float32).unsqueeze(0).to(
            self.device)  # 48,11
        ...
        indice0 = torch.min(torch.max(torch.FloatTensor([1]).to(self.device), indice0),
                            torch.FloatTensor([in_size[0]]).to(self.device)).unsqueeze(0)
        indice1 = torch.min(torch.max(torch.FloatTensor([1]).to(self.device), indice1),
                            torch.FloatTensor([in_size[1]]).to(self.device)).unsqueeze(0)
    ...
    def __call__(self, batch, random, kernel_size):
        ...
        # stable kernel
            ...
            return stable_gaussian_kernel(device=self.device, kernel_size=kernel_size, blur_type=self.blur_type,
                                          sig=self.sig, lambda_1=self.lambda_1, lambda_2=self.lambda_2,
                                          theta=self.theta)
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, img, scale, norm=True):
        with torch.no_grad():
            ...
            # downsampling
            avg_pool2d = nn.AvgPool2d(kernel_size=(scale, 1), stride=(scale, 1))
            lr_blured = avg_pool2d(hr_blured)
