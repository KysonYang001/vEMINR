...
DUMMY_SDF_PATH_TRAIN = 'dummy_train_sdf.tif'
DUMMY_IMG_PATH_VAL = 'dummy_val_img.tif'
DUMMY_SDF_PATH_VAL = 'dummy_val_sdf.tif'
CONFIG_DIR = 'configs'

TEST_PARAMS = {
    'img_shape': (16, 64, 64), # (slices, height, width)
    'batch_size': 2,
    'inp_size': 24,
    'sample_q': 1024,
    'scale': 2,
...
def create_dummy_files():
    ...
    # 创建训练数据
    print(f"创建训练图像: {DUMMY_IMG_PATH_TRAIN}")
    ...
    io.imsave(DUMMY_SDF_PATH_TRAIN, dummy_sdf_train)

    # 创建验证数据
    print(f"创建验证图像: {DUMMY_IMG_PATH_VAL}")
    ...
    io.imsave(DUMMY_SDF_PATH_VAL, dummy_sdf_val)
...
train_dataset1:
  dataset:
    name: image-volume
    args:
      root_path: {DUMMY_IMG_PATH_TRAIN}
      sdf_path: {DUMMY_SDF_PATH_TRAIN}
      repeat: 1
  wrapper:
    name: sr-gaussian
    args:
...
val_dataset1:
  dataset:
    name: image-volume
    args:
      root_path: {DUMMY_IMG_PATH_VAL}
      sdf_path: {DUMMY_SDF_PATH_VAL}
  wrapper:
    name: sr-gaussian
    args:
      inp_size: {TEST_PARAMS['inp_size']}
...
sdf_norm:
  sub: [{sdf_mean}]
  div: [{sdf_std if sdf_std > 1e-6 else 1.0}]
...
def run_pipeline_test():
    ...
    # 2. 模型和优化器
    # 注意：我们将degrade模型的路径设为None，因此它将随机初始化
    config['model']['degrade']['path'] = None 
    ...
    print("\n执行前向传播...")
    optimizer.zero_grad()
    preds = model(lr, coord, cell, state='train')
    ...
    # 加载SDF归一化参数用于反归一化
    t = config['sdf_norm']
    sdf_sub = torch.FloatTensor(t['sub']).view(1, 1, -1).to(device)
    sdf_div = torch.FloatTensor(t['div']).view(1, 1, -1).to(device)

    pred_sdf_orig = pred_sdf * sdf_div + sdf_sub
    gt_sdf_orig = gt_sdf * sdf_div + sdf_sub
    loss_geom = geom_criterion(pred_sdf_orig, gt_sdf_orig)
    
    total_loss = loss_sr + config['lambda_geom'] * loss_geom
    print(f"  - SR Loss: {loss_sr.item():.4f}")
    print(f"  - Geom Loss: {loss_geom.item():.4f}")
