...
class APT(nn.Module):
    def __init__(
            self, nin=256, nout=1, bias=True, act=nn.ReLU(True)):
        super(APT, self).__init__()

        self.mod = nn.Sequential(nn.Linear(nin, nout, bias=bias),
                                 nn.LeakyReLU(),
...
class LIIF(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None,
        ...
        if imnet_spec is not None:
            ...
            if self.feat_unfold:
                imnet_in_dim *= 9
            imnet_in_dim += 64 * 2 # attach coord
            if self.cell_decode:
                imnet_in_dim += 2
            if self.degrade:
                imnet_in_dim += 1
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                if self.degrade:
                    ...
                    vector_degrade = vector_degrade.unsqueeze(1).repeat(1, inp.shape[-2], 1)
