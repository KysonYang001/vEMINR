...
def save(model, optimizer, opspec, epoch, DDP, savepath, state):
    ...
    elif state=='best-loss':
        torch.save(sv_file, os.path.join(savepath, 'epoch-best-loss.pth'))

    if state == 'hun':
        state = 'epoch-{}'.format(epoch)
    print(f'save in {os.path.join(savepath, state)}.pth')
...
def wave(inp, xfm):
    if inp.shape[-1]==1 or inp.shape[-1]==3:
        c = inp.shape[-1]
        tem = int(math.sqrt(inp.shape[-2]))
        inp = inp.permute(0, 2, 1).reshape(-1, c, tem, tem)
    # wavelet transform, to encoder
    c = inp.shape[1]
    LL, Yh = xfm(inp.view(-1, c, inp.shape[-2], inp.shape[-1]))
    HL, LH, HH = torch.unbind(Yh[0], dim=2)
    LL = LL.reshape(-1, c, LL.shape[-2], LL.shape[-1])
    HH = HH.reshape(-1, c, HH.shape[-2], HH.shape[-1])
    LH = LH.reshape(-1, c, LH.shape[-2], LH.shape[-1])
    HL = HL.reshape(-1, c, HL.shape[-2], HL.shape[-1])
    # H = torch.cat((LL, HH, LH, HL), dim=1)
